...
if ( !class_exists( 'WP_Settings_Settings' ) ) {
	class WP_Settings_Settings {
		 ...
		 *
		 * @uses WP_Settings_Form_Fields class
		 *
		...
		protected function settings_admin_init( $pages, $fields, $page_hook = '' ) {
			...
			if ( !class_exists( 'WP_Settings_Form_Fields' ) )
				$this->debug  .= "Error: class WP_Settings_Form_Fields doesn't exist<br/>";

			if ( '' == $page_hook )
				$this->debug  .= "Error: parameter 'page_hook' not provided in settings_admin_init()<br/>";

			$this->debug = apply_filters( 'wp_settings_debug', $this->debug, $pages, $fields, $page_hook );
			...
			foreach ( $page['sections'] as $sections ) {
				...
				if ( isset( $this->fields[ $sections['id'] ] ) ) {
					...
					foreach ( $this->fields[ $sections['id'] ] as $option ) {
						...
						// add the fields
						$form_fields =  new WP_Settings_Form_Fields( get_settings_errors() );
		...
		protected function render_form( $form = '' ) {
			...
			if ( !empty( $page ) ) {
				...
				foreach ( $forms as $form ) {
					...
					echo apply_filters( 'wp_settings_form', '', $form['id'] );
